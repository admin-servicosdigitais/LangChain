...
class TestQuotaTracker:
    ...
    @pytest.mark.asyncio
    async def test_normal_usage_returns_none(self):
        table = self._make_table(100_000)
        tracker = QuotaTracker(table, monthly_limit=500_000)
        result = await tracker.increment(100)
    ...
    async def test_warning_at_80_pct(self):
        ...
        tracker = QuotaTracker(table, monthly_limit=500_000)
        result = await tracker.increment(1)
        assert result == AlertLevelEnum.WARNING

    @pytest.mark.asyncio
    async def test_critical_at_95_pct(self):
    ...
    async def test_get_current_usage(self):
        ...
        tracker = QuotaTracker(table, monthly_limit=500_000)
        count, month = await tracker.get_current_usage()
